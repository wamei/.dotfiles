...
(defun ah:title-real-to-display (bufferName)
  (with-current-buffer (get-buffer bufferName)
    (if (and howm-mode
             (ah:in-howm-dir-p bufferName))
        (ah:title-get-title bufferName)
      bufferName))
  )

(defun ah:in-howm-dir-p (bufferName)
  (let ((file-name (buffer-file-name (get-buffer bufferName))))
    (if file-name
        (ah:!! (string-match ah:howm-full-path-directory file-name))
      nil
      )))
